...
async function handleJob(job) {
  ...
  if (
    ...
    queue.getUserQueuedAmt(job.ip) + job.remainingSize() >
  ...
  if (
    ...
    queue.size() + job.remainingSize() > CONFIG.max_queue_size
  ) {
...
queue.process(CONFIG.logins.length, botController, async (job) => {
  ...
  itemData.iteminfo.keychains = itemData.iteminfo.keychains.map((s) =>
    utils.removeNullValues(s)
  );
